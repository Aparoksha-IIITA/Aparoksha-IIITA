...
import Force3 from "../common/oldForce3";
import ForceCamera from "../common/ForceCamera";
import ForceHemisphereLight from "../common/ForceHemisphereLight";
import Util from "../common/util";

import img1 from "../../assets/images/gallery/image01.jpg";
...
export default function gallery() {
  ...
  var getNearImage = function (camera, image) {
    get_near = true;
    camera.force.position.anchor.set(
      Math.cos(image.rad) * 780,
      image.obj.position.y,
      Math.sin(image.rad) * 780
    );
  ...
  const render = () => {
    ...
    if (get_near === false) {
      camera.force.look.anchor.copy(
        Util.getPolarCoord(camera.rotate_rad1, camera.rotate_rad2, 1000)
      );
    }
  ...
  const on = () => {
    ...
    const touchMove = (x, y, touch_event) => {
      ...
      if (
        is_clicked &&
        vectorTouchStart.clone().sub(vectorTouchMove).length() > 0.01
      ) {
        is_clicked = false;
        is_draged = true;
      }
      if (is_draged == true && get_near == false) {
        camera.rotate_rad1 =
          camera.rotate_rad1_base +
          Util.getRadian((vectorTouchStart.y - vectorTouchMove.y) * 50);
        camera.rotate_rad2 =
          camera.rotate_rad2_base +
          Util.getRadian((vectorTouchStart.x - vectorTouchMove.x) * 50);
